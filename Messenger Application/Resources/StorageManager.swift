...
//
//  Created by Safa Falaqi on 05/01/2022.
//
...
final class StorageManager {
    ...
    /*
     /images/safa-gmail-com_profile_picture.png - using this storage object
     */
    ...
    public func uploadConversationImage(with data: Data, fileName: String, completion: @escaping UploadPictureCompletion){
        storage.child("message_image/\(fileName)").putData(data, metadata: nil) { metadata, error in
            guard error == nil else {
                // failed
            ...
            self.storage.child("message_image/\(fileName)").downloadURL { url, error in
                guard let url = url else {
                    print("Failed to get download url")
                    completion(.failure(StorageErrors.failedToGetDownloadUrl))
                    return
                ...
                let urlString = url.absoluteString
                print("download url returned: \(urlString)")
                completion(.success(urlString)) //return url
            }
        }
